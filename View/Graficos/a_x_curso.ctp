       ...
       $this->Session->setFlash(__('Could not connect to database.'), 'default', array('class' => 'error'));
        ...
        while ($i < $numcursos) {
               ...
               if (!in_array($curso_id, $id_repetida)) {
                        ...
                        $cursos_inscripcions = $db->Query("SELECT curso_id,inscripcion_id,recursante FROM `cursos_inscripcions` LEFT JOIN inscripcions ON cursos_inscripcions.inscripcion_id = inscripcions.id where inscripcions.fecha_baja != 'NULL' and curso_id = '$curso_id'");
